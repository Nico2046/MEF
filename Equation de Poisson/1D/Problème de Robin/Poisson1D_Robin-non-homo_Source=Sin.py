...
# -*- coding: utf-8 -*-
"""
Created on Sat Mar 29 17:09:11 2025
...
Equation de Poisson 1D :
                         -u''(x) = f(x) sur 𝛀 = ]0, L[ (L > 0) avec f(x) = sin(x)
...
for i in range(E): # La boucle parcourt chaque élément
    ...
    # Assemblage du vecteur élémentaire (La méthode d'intégration numérique est la méthode de Simpson composite)
...
def u_exact(x_range):
    return (beta_0 * beta_L + beta_L * g_0 + beta_0 * g_L + alpha_L * beta_0 * L + alpha_L * g_0 * L - 
   alpha_L * beta_0 * x_range - alpha_L * g_0 * x_range + alpha_0 * g_L * x_range - beta_0 * beta_L * np.cos(L) - 
   alpha_0 * beta_L * x_range * np.cos(L) - alpha_L * beta_0 * np.sin(L) - 
   alpha_0 * alpha_L * x_range * np.sin(L) + alpha_L * beta_0 * np.sin(x_range) + 
   alpha_0 * beta_L * np.sin(x_range) + alpha_0 * alpha_L * L * np.sin(x_range))/(alpha_L * beta_0 + 
   alpha_0 * beta_L + alpha_0 * alpha_L * L)
